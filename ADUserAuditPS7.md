...
PowerShell 7 (Windows) Compatible
...
- Lines containing "✓" print in green
- Only matches Archive-Security-YYYY-MM-DD-HH-MM-SS-fff.evtx
- Strict YYYY-MM-DD parsing; End date ENTER = today
- "Newest → Oldest": process CURRENT first, then archived (newest→oldest)
- No-match is NOT an error: export empty CSV and print green "✓ ... (0 rows)"
#>

$ErrorActionPreference = 'Stop'

# -------- Platform guard (PS7 on Windows) --------
...
# -------- Script directory (PS7-safe) --------
$ScriptPath = $PSCommandPath
$ScriptDir  = if ([string]::IsNullOrWhiteSpace($ScriptPath)) { (Get-Location).Path } else { Split-Path -Parent $ScriptPath }
...
function Test-AdminOrELR {
    try{
        ...
        $isELR=$false
        foreach($g in $wi.Groups){ if($g.Translate([Security.Principal.SecurityIdentifier]).Value -eq $elrSid.Value){ $isELR=$true; break } }
...
function Prompt-AbsolutePath([string]$prompt,[string]$default){
    ...
    $isAbs=($in -match '^[a-zA-Z]:\\') -or ($in -match '^\\\\')
    if(-not $isAbs){ throw "Path must be an absolute path: '$in'" }
...
function Confirm-StartEnd([datetime]$start,[datetime]$end){ if($start -gt $end){ throw "Start date cannot be later than end date." } }
...
function Select-EventFields {
    ...
    $target=(& $getVal 'TargetUserName'); if(-not $target){ $target=(& $getVal 'AccountName') }
    $subject=(& $getVal 'SubjectUserName'); if(-not $subject){ $subject=(& $getVal 'SubjectAccountName') }
...
function Get-FileTimeWindow([string]$path){
    try{
        ...
        if($oldest -and $newest){ return [pscustomobject]@{ Path=$path; Start=$oldest.TimeCreated.Date; End=$newest.TimeCreated.Date; Valid=$true } }
    }catch{}
...
Write-Info "# PowerShell 7 (Windows)"
Write-Info "# =======================`n"
...
Write-Info "6) Account Enabled + Disabled     (4722+4725)"
...
if($logSel -eq '2'){
    try{
        ...
        $windows=foreach($fItem in $archCandidates){ $w=Get-FileTimeWindow -path $fItem.FullName; if($w.Valid){ $w } }
